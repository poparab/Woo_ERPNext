from __future__ import annotations

import frappe


REQUIRED_FIELDS = [
    # Sales Invoice fields
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_order_id",
        "fieldtype": "Int",
        "label": "Woo Order ID",
        "insert_after": "title",
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_order_number",
        "fieldtype": "Data",
        "label": "Woo Order Number",
        "insert_after": "woo_order_id",
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_transaction_id",
        "fieldtype": "Data",
        "label": "Woo Transaction ID",
        "insert_after": "woo_order_number",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_order_display_name",
        "fieldtype": "Data",
        "label": "Woo Order Display Name",
        "insert_after": "woo_transaction_id",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_billing_name",
        "fieldtype": "Data",
        "label": "Woo Billing Name",
        "insert_after": "woo_order_display_name",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_shipping_name",
        "fieldtype": "Data",
        "label": "Woo Shipping Name",
        "insert_after": "woo_billing_name",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_order_phone",
        "fieldtype": "Data",
        "label": "Woo Order Phone",
        "insert_after": "woo_shipping_name",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_order_phone_normalized",
        "fieldtype": "Data",
        "label": "Woo Order Phone Normalized",
        "insert_after": "woo_order_phone",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_order_email",
        "fieldtype": "Data",
        "label": "Woo Order Email",
        "insert_after": "woo_order_phone_normalized",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_customer_id_snapshot",
        "fieldtype": "Data",
        "label": "Woo Customer ID Snapshot",
        "insert_after": "woo_order_email",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_contact_hash",
        "fieldtype": "Data",
        "label": "Woo Contact Hash",
        "insert_after": "woo_customer_id_snapshot",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "sales_invoice_state",
        "fieldtype": "Data",
        "label": "Sales Invoice State",
        "insert_after": "status",
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_outbound_status",
        "fieldtype": "Data",
        "label": "Woo Outbound Status",
        "insert_after": "sales_invoice_state",
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_outbound_error",
        "fieldtype": "Small Text",
        "label": "Woo Outbound Error",
        "insert_after": "woo_outbound_status",
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_outbound_synced_on",
        "fieldtype": "Datetime",
        "label": "Woo Outbound Synced On",
        "insert_after": "woo_outbound_error",
    },
    # Sales Invoice Item bundle linkage (used to reconstruct safe outbound/inbound bundle shapes)
    {
        "dt": "Sales Invoice Item",
        "fieldname": "is_bundle_parent",
        "fieldtype": "Check",
        "label": "Is Bundle Parent",
        "insert_after": "discount_percentage",
        "hidden": 1,
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice Item",
        "fieldname": "is_bundle_child",
        "fieldtype": "Check",
        "label": "Is Bundle Child",
        "insert_after": "is_bundle_parent",
        "hidden": 1,
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice Item",
        "fieldname": "bundle_code",
        "fieldtype": "Data",
        "label": "Bundle Code",
        "insert_after": "is_bundle_child",
        "hidden": 1,
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice Item",
        "fieldname": "parent_bundle",
        "fieldtype": "Data",
        "label": "Parent Bundle",
        "insert_after": "bundle_code",
        "hidden": 1,
        "read_only": 1,
        "no_copy": 1,
    },
    # --- Marketing Attribution (from WooCommerce Order Attribution plugin) ---
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_attribution_section",
        "fieldtype": "Section Break",
        "label": "Marketing Attribution",
        "insert_after": "woo_outbound_synced_on",
        "collapsible": 1,
        "read_only": 1,
    },
    # Column 1: Campaign / UTM data
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_source_type",
        "fieldtype": "Data",
        "label": "Source Type",
        "insert_after": "woo_attribution_section",
        "read_only": 1,
        "no_copy": 1,
        "in_standard_filter": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_utm_source",
        "fieldtype": "Data",
        "label": "UTM Source",
        "insert_after": "woo_source_type",
        "read_only": 1,
        "no_copy": 1,
        "in_standard_filter": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_utm_medium",
        "fieldtype": "Data",
        "label": "UTM Medium",
        "insert_after": "woo_utm_source",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_utm_campaign",
        "fieldtype": "Data",
        "label": "UTM Campaign",
        "insert_after": "woo_utm_medium",
        "read_only": 1,
        "no_copy": 1,
        "in_standard_filter": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_utm_content",
        "fieldtype": "Data",
        "label": "UTM Content",
        "insert_after": "woo_utm_campaign",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_utm_term",
        "fieldtype": "Data",
        "label": "UTM Term",
        "insert_after": "woo_utm_content",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_utm_id",
        "fieldtype": "Data",
        "label": "UTM ID",
        "insert_after": "woo_utm_term",
        "read_only": 1,
        "no_copy": 1,
    },
    # Column 2: Session & Device data
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_attribution_col_break",
        "fieldtype": "Column Break",
        "insert_after": "woo_utm_id",
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_referrer",
        "fieldtype": "Data",
        "label": "Referrer",
        "insert_after": "woo_attribution_col_break",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_device_type",
        "fieldtype": "Data",
        "label": "Device Type",
        "insert_after": "woo_referrer",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_session_entry",
        "fieldtype": "Data",
        "label": "Landing Page",
        "insert_after": "woo_device_type",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_session_start",
        "fieldtype": "Data",
        "label": "Session Start",
        "insert_after": "woo_session_entry",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_session_pages",
        "fieldtype": "Int",
        "label": "Session Pages",
        "insert_after": "woo_session_start",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_session_count",
        "fieldtype": "Int",
        "label": "Session Count",
        "insert_after": "woo_session_pages",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Sales Invoice",
        "fieldname": "woo_user_agent",
        "fieldtype": "Small Text",
        "label": "User Agent",
        "insert_after": "woo_session_count",
        "read_only": 1,
        "no_copy": 1,
    },
    # Item field mapping to Woo product (id or sku mapping reference)
    {
        "dt": "Item",
        "fieldname": "woo_product_id",
        "fieldtype": "Data",
        "label": "Woo Product ID",
        "insert_after": "item_name",
    },
    {
        "dt": "Item",
        "fieldname": "woo_variation_id",
        "fieldtype": "Data",
        "label": "Woo Variation ID",
        "insert_after": "woo_product_id",
    },
    {
        "dt": "Customer",
        "fieldname": "woo_customer_id",
        "fieldtype": "Data",
        "label": "Woo Customer ID",
        "insert_after": "customer_group",
    },
    {
        "dt": "Customer",
        "fieldname": "woo_outbound_status",
        "fieldtype": "Data",
        "label": "Woo Outbound Status",
        "insert_after": "woo_customer_id",
    },
    {
        "dt": "Customer",
        "fieldname": "woo_outbound_error",
        "fieldtype": "Small Text",
        "label": "Woo Outbound Error",
        "insert_after": "woo_outbound_status",
    },
    {
        "dt": "Customer",
        "fieldname": "woo_outbound_synced_on",
        "fieldtype": "Datetime",
        "label": "Woo Outbound Synced On",
        "insert_after": "woo_outbound_error",
    },
    # --- Customer First-Touch Attribution ---
    {
        "dt": "Customer",
        "fieldname": "woo_first_attribution_section",
        "fieldtype": "Section Break",
        "label": "First Order Attribution",
        "insert_after": "woo_outbound_synced_on",
        "collapsible": 1,
        "read_only": 1,
    },
    {
        "dt": "Customer",
        "fieldname": "woo_first_source_type",
        "fieldtype": "Data",
        "label": "First Source Type",
        "insert_after": "woo_first_attribution_section",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Customer",
        "fieldname": "woo_first_utm_source",
        "fieldtype": "Data",
        "label": "First UTM Source",
        "insert_after": "woo_first_source_type",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Customer",
        "fieldname": "woo_first_utm_medium",
        "fieldtype": "Data",
        "label": "First UTM Medium",
        "insert_after": "woo_first_utm_source",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Customer",
        "fieldname": "woo_first_utm_campaign",
        "fieldtype": "Data",
        "label": "First UTM Campaign",
        "insert_after": "woo_first_utm_medium",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Customer",
        "fieldname": "woo_first_attribution_col_break",
        "fieldtype": "Column Break",
        "insert_after": "woo_first_utm_campaign",
    },
    {
        "dt": "Customer",
        "fieldname": "woo_first_referrer",
        "fieldtype": "Data",
        "label": "First Referrer",
        "insert_after": "woo_first_attribution_col_break",
        "read_only": 1,
        "no_copy": 1,
    },
    {
        "dt": "Customer",
        "fieldname": "woo_first_device_type",
        "fieldtype": "Data",
        "label": "First Device Type",
        "insert_after": "woo_first_referrer",
        "read_only": 1,
        "no_copy": 1,
    },
    # Jarz Bundle mapping to Woo bundle id
    {
        "dt": "Jarz Bundle",
        "fieldname": "woo_bundle_id",
        "fieldtype": "Data",
        "label": "Woo Bundle ID",
        "insert_after": "bundle_price",
    },
    # Territory -> POS Profile mapping (used to set Sales Invoice.pos_profile)
    {
        "dt": "Territory",
        "fieldname": "pos_profile",
        "fieldtype": "Link",
        "options": "POS Profile",
        "label": "POS Profile",
        "insert_after": "territory_manager",
    },
    # Territory -> Delivery Income (used to add an 'Actual' shipping income row)
    {
        "dt": "Territory",
        "fieldname": "delivery_income",
        "fieldtype": "Currency",
        "label": "Delivery Income",
        "insert_after": "pos_profile",
    },
    # Company -> Kashier account (used for Kashier Card / Kashier Wallet Payment Entries)
    {
        "dt": "Company",
        "fieldname": "custom_kashier_account",
        "fieldtype": "Link",
        "options": "Account",
        "label": "Kashier Account",
        "insert_after": "default_bank_account",
    },
]


def ensure_custom_fields():  # pragma: no cover - install / migration helper
    """Create required custom fields if they don't exist."""
    # Remove legacy duplicate Item custom field if both exist (e.g., 'Woo Product ID' vs 'woo_product_id')
    try:
        # Legacy variants by label or alternate fieldname patterns
        legacy_label_variants = [
            "Woo Product Id",  # capitalization variant
            "Woo Product ID",  # duplicate label variant (kept but if double we prefer primary fieldname)
            "woocommerce_product_id",
            "woo_product_id"  # lowercase label variant (if mis-entered as label)
        ]
        legacy_fieldname_variants = [
            "custom_woo_product_id",
            "woocommerce_product_id",
        ]
        primary_cf_name = "Item-woo_product_id"
        if frappe.db.exists("Custom Field", primary_cf_name):
            # Delete by label
            for legacy_label in legacy_label_variants:
                duplicates = frappe.get_all(
                    "Custom Field",
                    filters={"dt": "Item", "label": legacy_label},
                    pluck="name",
                )
                for dup in duplicates:
                    if dup != primary_cf_name:
                        frappe.delete_doc("Custom Field", dup, ignore_permissions=True)
            # Delete by fieldname
            for legacy_fn in legacy_fieldname_variants:
                if legacy_fn != "woo_product_id":
                    cf_name = f"Item-{legacy_fn}"
                    if frappe.db.exists("Custom Field", cf_name):
                        frappe.delete_doc("Custom Field", cf_name, ignore_permissions=True)
    except Exception:
        frappe.log_error(frappe.get_traceback(), "Woo Integration: cleanup duplicate item custom fields")

    for spec in REQUIRED_FIELDS:
        dt = spec["dt"]
        fn = spec["fieldname"]
        # Skip if already present in DocType meta (standard or previously added custom field)
        try:
            meta = frappe.get_meta(dt)
            if meta and meta.get_field(fn):
                continue
        except Exception:
            pass
        # Skip if any Custom Field exists with same dt+fieldname (regardless of name)
        try:
            exists_same_fn = frappe.get_all("Custom Field", filters={"dt": dt, "fieldname": fn}, limit=1)
            if exists_same_fn:
                continue
        except Exception:
            pass
        # Skip if the conventional name exists
        if frappe.db.exists("Custom Field", f"{dt}-{fn}"):
            continue
        cf = frappe.get_doc({
            "doctype": "Custom Field",
            "dt": dt,
            "fieldname": fn,
            "fieldtype": spec["fieldtype"],
            "label": spec.get("label", ""),
            "insert_after": spec.get("insert_after"),
            "options": spec.get("options"),
            "collapsible": spec.get("collapsible", 0),
            "hidden": spec.get("hidden", 0),
            "read_only": spec.get("read_only", 0),
            "no_copy": spec.get("no_copy", 0),
            "in_standard_filter": spec.get("in_standard_filter", 0),
        })
        cf.insert(ignore_permissions=True)
    frappe.clear_cache()
