frappe.ui.form.on('Sales Invoice', {
    refresh(frm) {
        ...
        // Add "Push to WooCommerce" button (always visible on submitted invoices)
        const btn = frm.add_custom_button(__('Push to WooCommerce'), function () {
            frappe.confirm(
                __('Push this Sales Invoice to WooCommerce now? This will override any pending or failed sync.'),
                function () {
                    frappe.show_alert({ message: __('Pushing to WooCommerce…'), indicator: 'blue' });
                    frappe.call({
                        method: 'jarz_woocommerce_integration.api.manual_sync.push_sales_invoice',
                        ...
                        callback(r) {
                            ...
                            if (result.status === 'ok') {
                                frappe.show_alert({
                                    message: __('Pushed successfully (Woo Order #{0})', [result.woo_order_id || '?']),
                                    indicator: 'green',
                                });
