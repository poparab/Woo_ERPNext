...
class TestHtml200FailsSoftWithDiagnosis(unittest.TestCase):
    """A 200 carrying an HTML WAF page retries, then raises a diagnosable transient."""

    def test_html_200_retries_then_raises_transient_with_body_snippet(self):
        client, session = _client([_html_response(), _html_response(), _html_response()])

        with self.assertRaises(WooTransientError) as excinfo:
        ...
        assert "\n" not in message
        assert excinfo.exception.status_code == 200
        assert excinfo.exception.payload["content_type"] == "text/html; charset=UTF-8"
        assert excinfo.exception.payload["attempts"] == 3

    def test_transient_error_is_a_woo_api_error_subclass(self):
        """Existing `except WooAPIError` handlers must keep working unchanged."""
        assert issubclass(WooTransientError, WooAPIError)

    def test_body_snippet_is_bounded_and_collapsed(self):
    ...
    def test_empty_body_snippet_is_labelled(self):
        assert http_client.body_snippet(FakeResponse(status_code=200, text="")) == "<empty-body>"

    def test_snippets_and_urls_never_leak_credentials(self):
...
class TestValidJsonNeedsNoRetry(unittest.TestCase):
    """The happy path costs exactly one call."""

    def test_valid_json_succeeds_first_try(self):
...
class TestRetryThenRecoverOrExhaust(unittest.TestCase):
    """A transient first attempt recovers on the retry; three of them exhaust."""

    def test_transient_html_then_valid_json_succeeds(self):
    ...
    def test_429_then_valid_json_succeeds(self):
        orders = [{"id": 9}]
    ...
    def test_502_exhausts_retries_and_raises_transient(self):
        def bad_gateway():
    ...
    def test_connection_error_retries_then_reraises_original(self):
...
class TestAuthAndNotFoundStayLoud(unittest.TestCase):
    """401/403/404 are real errors: raised at once, never softened to transient."""

    def test_auth_failure_raises_immediately_without_retries(self):
        for status_code in (401, 403):
            with self.subTest(status_code=status_code):
                ...
                with self.assertRaises(WooAPIError) as excinfo:
                    client.list_orders_with_meta(params={"per_page": 10})

                # Genuine auth errors stay loud: a hard WooAPIError, NOT a soft transient one.
                assert not isinstance(excinfo.exception, WooTransientError)
                assert excinfo.exception.status_code == status_code
                assert "cannot list resources" in str(excinfo.exception)
                assert len(session.calls) == 1  # no pointless retries

    def test_404_raises_immediately_without_retries(self):
        client, session = _client(
...
class TestPlainPermalinkRestRouteFallback(unittest.TestCase):
    """An HTML 404 on /wp-json/ means Plain permalinks -- fall back to ?rest_route=."""

    def test_html_404_falls_back_to_rest_route_and_succeeds(self):
    ...
    def test_rest_route_mode_is_sticky_after_first_fallback(self):
    ...
    def test_json_404_is_a_real_error_and_does_not_trigger_fallback(self):
        """"Order not found" is a legitimate JSON 404 — never a routing problem."""
        client, session = _client(
            [FakeResponse(status_code=404, text='{"message": "Invalid ID."}', json_body={"message": "Invalid ID."})]
        )

        with self.assertRaises(WooAPIError):
            client.list_orders_with_meta(params={"per_page": 10})

        assert client.use_rest_route is False
        assert len(session.calls) == 1

    def test_html_200_does_not_trigger_fallback(self):
...
class TestUrlBuilding(unittest.TestCase):
    """Both URL forms, and the auth marker the fallback depends on."""

    def test_build_url_rest_route_forms(self):
        cases = [
    ...
    def test_rest_route_url_always_carries_the_wc_auth_marker(self):
    ...
    def test_build_url_pretty_forms_are_unchanged(self):
        client, _session = _client([])
    ...
    def test_get_order_still_swallows_transient_and_returns_none(self):
...
class TestBackoffShape(unittest.TestCase):
    """Exponential, bounded, jittered, and honouring Retry-After."""

    def setUp(self):
        self.monkeypatch = MonkeyPatch()
        self.addCleanup(self.monkeypatch.undo)

    def test_backoff_grows_exponentially_and_is_bounded_and_jittered(self):
        base = 1.0
    ...
    def test_sleep_is_skipped_when_backoff_disabled(self):
        slept = []
        self.monkeypatch.setattr(http_client.time, "sleep", lambda s: slept.append(s))
        client, _session = _client([_html_response(), _html_response(), _html_response()])
        with self.assertRaises(WooTransientError):
            client.list_orders_with_meta(params={"per_page": 10})
        assert slept == []

    def test_sleep_happens_between_retries_when_backoff_enabled(self):
        slept = []
        self.monkeypatch.setattr(http_client.time, "sleep", lambda s: slept.append(s))
        client, session = _client(
...
class TestOrderSyncFailsSoftOnTransient(unittest.TestCase):
    """A transient must skip the window, not abort the whole sync run."""

    def setUp(self):
        self.monkeypatch = MonkeyPatch()
        self.addCleanup(self.monkeypatch.undo)

    def test_enqueue_order_window_events_fails_soft_on_transient(self):
        """A flaky store must not abort the sync or raise to Sentry."""
        from types import SimpleNamespace

        warnings = _patch_window_raising_transient(self.monkeypatch)
    ...
    def test_pull_recent_orders_phase1_fails_soft_on_transient(self):
        from types import SimpleNamespace
    ...
    def test_transient_skip_detail_carries_no_credentials(self):
        warnings = _patch_window_raising_transient(self.monkeypatch)
...
class TestSyncEventsClassifiesTransient(unittest.TestCase):
    """A transient is retryable; an auth failure still goes to NeedsReview."""

    def setUp(self):
        self.monkeypatch = MonkeyPatch()
        self.addCleanup(self.monkeypatch.undo)

    def test_sync_events_reschedules_transient_error(self):
    ...
    def test_sync_events_still_flags_auth_error_for_review(self):
    ...
    def test_non_transient_woo_error_still_propagates(self):
        ...
        self.monkeypatch.setattr(order_sync, "_list_orders_window", fake_list_orders_window)
