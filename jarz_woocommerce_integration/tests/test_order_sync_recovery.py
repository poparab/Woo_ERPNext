...
class TestOrderSyncRecovery(unittest.TestCase):
    """Cursor tracking, backfill aggregation and contact/territory snapshot recovery."""

    def setUp(self):
        self.monkeypatch = MonkeyPatch()
        self.addCleanup(self.monkeypatch.undo)

    def test_minutes_ago_for_woo_uses_real_utc(self):
        self.monkeypatch.setattr(order_sync, "datetime", FrozenDateTime)

        assert order_sync._minutes_ago_for_woo(30) == "2026-05-01T13:21:04Z"

    def test_pull_recent_orders_phase1_tracks_cursor_and_skip_reasons(self):
        captured = {}
        orders = [
        ...
        self.monkeypatch.setattr(order_sync, "WooClient", DummyClient)
        ...
        self.monkeypatch.setattr(order_sync, "process_order_phase1", fake_process)
        self.monkeypatch.setattr(order_sync.frappe.db, "commit", lambda: commit_calls.append(True))
    ...
    def test_backfill_orders_by_ids_phase1_aggregates_statuses(self):
        results = {
    ...
    def test_process_order_phase1_normalizes_duplicate_customer_error(self):
        ...
        _patch_customer_failure_path(self.monkeypatch, duplicate)
    ...
    def test_process_order_phase1_normalizes_internal_customer_error(self):
    ...
    def test_refresh_order_contact_snapshot_updates_invoice_and_map(self):
        ...
        self.monkeypatch.setattr(order_sync.frappe.db, "set_value", fake_set_value)
        self.monkeypatch.setattr(order_sync.frappe.db, "commit", lambda: commit_calls.append(True))
        self.monkeypatch.setattr(order_sync.frappe.db, "get_table_columns", lambda doctype: ["erpnext_sales_invoice"])
        self.monkeypatch.setattr(order_sync.frappe.utils, "now_datetime", lambda: "2026-05-21 12:00:00")
    ...
    def test_order_territory_snapshot_prefers_shipping_state(self):
        calls = []
        ...
        cache = {}
        self.monkeypatch.setattr(order_sync, "_resolve_territory_from_state", fake_resolve)
    ...
    def test_order_map_snapshot_values_include_territory_snapshot(self):
        self.monkeypatch.setattr(order_sync.frappe.utils, "now_datetime", lambda: "2026-06-01 10:00:00")
    ...
    def test_process_order_phase1_sets_invoice_territory_from_order_state(self):
        created = {}
        map_docs = {}
        ...
        self.monkeypatch.setattr(order_sync, "get_redis_conn", lambda: DummyRedis())
        self.monkeypatch.setattr(order_sync.frappe.db, "sql", fake_sql)
        self.monkeypatch.setattr(order_sync.frappe.db, "get_table_columns", lambda doctype: ["erpnext_sales_invoice"])
        self.monkeypatch.setattr(order_sync.frappe.db, "get_value", fake_get_value)
        self.monkeypatch.setattr(order_sync.frappe.db, "exists", lambda doctype, name=None: False)
        self.monkeypatch.setattr(order_sync.frappe.db, "commit", lambda: None)
        self.monkeypatch.setattr(order_sync.frappe, "get_all", lambda *args, **kwargs: [])
        self.monkeypatch.setattr(order_sync.frappe, "get_doc", fake_get_doc)
        self.monkeypatch.setattr(order_sync.frappe, "flags", SimpleNamespace(ignore_woo_outbound=False))
        self.monkeypatch.setattr(order_sync.frappe.utils, "today", lambda: "2026-06-01")
        self.monkeypatch.setattr(order_sync.frappe.utils, "now_datetime", lambda: "2026-06-01 12:00:00")
        self.monkeypatch.setattr(
            order_sync,
            "ensure_customer_with_addresses",
        ...
        self.monkeypatch.setattr(order_sync, "_apply_delivery_charge_policy", lambda *args, **kwargs: {"changed": False})
        self.monkeypatch.setattr(order_sync, "_maybe_create_payment_entry_for_invoice", lambda *args, **kwargs: None)
        self.monkeypatch.setattr(order_sync, "_submit_invoice_with_accounting_guards", lambda *args, **kwargs: None)
    ...
    def test_refresh_order_contact_snapshot_dry_run_reports_without_writing(self):
        settings = SimpleNamespace(
            base_url="https://example.com",
            consumer_key="ck_test",
            get_password=lambda fieldname: "cs_test",
        ...
        self.monkeypatch.setattr(order_sync.frappe, "get_single", lambda doctype: settings)
        self.monkeypatch.setattr(order_sync.frappe.db, "get_value", fake_get_value)
        self.monkeypatch.setattr(order_sync.frappe.db, "set_value", lambda *args, **kwargs: writes.append((args, kwargs)))
        self.monkeypatch.setattr(order_sync.frappe.db, "commit", lambda: writes.append("commit"))
        self.monkeypatch.setattr(order_sync.frappe.db, "get_table_columns", lambda doctype: ["erpnext_sales_invoice"])
    ...
    def test_backfill_order_contact_snapshots_by_ids_aggregates_statuses(self):
        results = {
    ...
    def test_reconcile_recent_orders_phase1_uses_modified_after_window(self):
        captured = {}
        settings = SimpleNamespace(
        ...
        self.monkeypatch.setattr(order_sync, "pull_recent_orders_phase1", fake_pull_recent_orders_phase1)
    ...
    def test_pull_recent_orders_phase1_filters_by_status_filter_set(self):
    ...
    def test_reconcile_recent_orders_phase1_sends_any_to_woo(self):
    ...
    def test_sync_cancelled_orders_cron_uses_any_with_local_status_filter(self):
        captured = {}
        ...
        self.monkeypatch.setattr(order_sync.frappe.db, "get_single_value", lambda doctype, fieldname: 1)
        self.monkeypatch.setattr(order_sync, "_run_order_cursor_sync", fake_run_order_cursor_sync)
