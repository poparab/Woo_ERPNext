...
def test_order_webhook_shadow_insert_failure_keeps_direct_enqueue(monkeypatch):
    secret = "testsecret"
    ...
    monkeypatch.setattr(orders.frappe, "local", SimpleNamespace(response=SimpleNamespace(http_status_code=200)))
    monkeypatch.setattr(orders.frappe, "form_dict", {})
    monkeypatch.setattr(orders.frappe, "enqueue", lambda *args, **kwargs: enqueue_calls.append((args, kwargs)))
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.should_use_order_webhook_inbox", lambda settings: True)
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.create_inbound_order_event", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("shadow insert failed")))
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.is_shadow_mode_enabled", lambda settings: True)
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.report_shadow_insert_failure", lambda *args, **kwargs: shadow_failures.append((args, kwargs)))
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.enqueue_sync_event", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("event enqueue should not run")))
...
def test_customer_webhook_shadow_insert_failure_keeps_direct_enqueue(monkeypatch):
    secret = "testsecret"
    ...
    monkeypatch.setattr(webhooks.frappe, "local", SimpleNamespace(response=SimpleNamespace(http_status_code=200)))
    monkeypatch.setattr(webhooks.frappe, "form_dict", {})
    monkeypatch.setattr(webhooks.frappe, "enqueue", lambda *args, **kwargs: enqueue_calls.append((args, kwargs)))
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.should_use_customer_webhook_inbox", lambda settings: True)
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.create_inbound_customer_event", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("shadow insert failed")))
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.is_shadow_mode_enabled", lambda settings: True)
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.report_shadow_insert_failure", lambda *args, **kwargs: shadow_failures.append((args, kwargs)))
    monkeypatch.setattr("jarz_woocommerce_integration.services.sync_events.enqueue_sync_event", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("event enqueue should not run")))
