...
def _royal_feast_cache():
    """Cache covering the jar variations involved in the Woo 17278 incident."""
...
def _royal_feast_parent_line(woosb_ids: str) -> dict:
    return {
        "id": 61001,
...
def _royal_feast_child_line(line_id: int, name: str, variation_id: int, qty: int) -> dict:
    return {
        "id": line_id,
        "name": name,
...
#: The original mix on Woo order 17278 before the POS amendment: the string our
#: own outbound push then froze on the parent line.
_ROYAL_FEAST_STALE_WOOSB_IDS = (
...
def test_woo_17278_stale_parent_woosb_ids_loses_to_the_child_lines():
    """Regression for Woo order 17278 (2026-09-09).

    A POS operator amended ACC-SINV-2026-18202's Jarz Royal Feast from
    Pistachio 4 / Blueberry 4 / ChocHazelnut 1 / Redvelvet 1 to
    ChocHazelnut 4 / Pistachio 3 / Blueberry 3. The outbound push rewrote the Woo
    child lines to the new mix but preserved the store's `_woosb_ids`, which still
    stated the original. Reading `_woosb_ids` back rebuilt the OLD mix, looked like
    a customer edit, and produced an amendment that reverted the operator and
    shipped the wrong jars.
    """
    parent_line = _royal_feast_parent_line(_ROYAL_FEAST_STALE_WOOSB_IDS)
    line_items = [
        parent_line,
        _royal_feast_child_line(61002, "Chocolate Hazelnut", 13783, 4),
        _royal_feast_child_line(61003, "Pistachio", 13826, 3),
        _royal_feast_child_line(61004, "Blueberry", 13780, 3),
    ]

    selections = order_sync._build_bundle_selections(
        line_items, 123, 1, cache=_royal_feast_cache(), parent_line=parent_line
    )
...
def test_parent_woosb_ids_is_still_used_when_the_order_carries_no_child_lines():
    """Fallback intact: a bundle sent as a bare parent has only the selection string."""
    parent_line = _royal_feast_parent_line(_ROYAL_FEAST_STALE_WOOSB_IDS)

    selections = order_sync._build_bundle_selections(
        [parent_line], 123, 1, cache=_royal_feast_cache(), parent_line=parent_line
    )
...
def test_an_unmappable_child_falls_back_to_woosb_ids_not_to_bundle_defaults():
    """A half-readable child scan must not silently downgrade to the default mix.

    ``_build_bundle_selections_from_children`` returns ``{}`` for the whole bundle
    as soon as one child cannot be resolved, and an empty dict makes the caller
    expand bundle defaults — a mix nobody chose. The selection string is worse
    evidence than the children but far better than that.
    """
    parent_line = _royal_feast_parent_line(_ROYAL_FEAST_STALE_WOOSB_IDS)
    line_items = [
        parent_line,
        _royal_feast_child_line(61002, "Chocolate Hazelnut", 13783, 4),
        # Variation absent from the catalogue cache → unmappable.
        _royal_feast_child_line(61003, "Mystery Jar", 99999, 3),
    ]
