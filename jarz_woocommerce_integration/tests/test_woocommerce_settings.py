...
class TestWooCommerceSettingsApi(unittest.TestCase):
    """Webhook status reporting, repair/creation and the inbound-setup probe."""

    def setUp(self):
        self.monkeypatch = MonkeyPatch()
        self.addCleanup(self.monkeypatch.undo)

    def test_get_order_webhook_status_data_flags_missing_topic(self):
        ...
        self.monkeypatch.setattr(settings_api, "_get_expected_order_delivery_urls", lambda: [primary_url, legacy_url])
    ...
    def test_ensure_order_webhooks_repairs_and_creates(self):
        primary_url = "https://erp.example.com/api/method/jarz_woocommerce_integration.api.orders.woo_order_webhook"
        legacy_url = "https://erp.example.com/api/method/jarz_woocommerce_integration.api.webhook.order_webhook"
        ...
        self.monkeypatch.setattr(settings_api, "_get_saved_client", lambda: (settings, dummy_client))
        self.monkeypatch.setattr(settings_api, "_get_saved_webhook_secret", lambda _settings: "supersecret")
        self.monkeypatch.setattr(
            settings_api,
            "_build_delivery_url",
            lambda method: primary_url if method == settings_api.PRIMARY_ORDER_WEBHOOK_METHOD else legacy_url,
        ...
        self.monkeypatch.setattr(settings_api, "get_order_webhook_status", lambda: {"success": True, "all_required_active": True})
    ...
    def test_validate_inbound_setup_uses_shared_probe_and_reports_jobs(self):
        ...
        self.monkeypatch.setattr(settings_api, "_get_saved_client", lambda: (settings, object()))
        self.monkeypatch.setattr(settings_api, "_get_saved_webhook_secret", lambda _settings: "supersecret")
        self.monkeypatch.setattr(settings_api, "_get_order_webhook_status_data", lambda client: webhook_status)
        self.monkeypatch.setattr(settings_api.frappe, "get_all", lambda *args, **kwargs: scheduled_jobs)
        self.monkeypatch.setattr(settings_api, "_minutes_ago_for_woo", lambda minutes: "2026-05-01T12:51:04Z")

        def fake_pull_recent_orders_phase1(**kwargs):
            captured.update(kwargs)
            return {"orders_fetched": 0, "processed": 0, "errors": 0}

        self.monkeypatch.setattr(settings_api, "pull_recent_orders_phase1", fake_pull_recent_orders_phase1)
